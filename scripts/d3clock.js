var d3clock = function(config) {
  ...
  */
  ...
  var faces = {
      'sbb':{
          outerRing: {r:outerRadius * 1.05, stroke: 'grey', strokeWidth: 2},
          secondsRing: {r:outerRadius * 0, fill: 'grey'},
          secondsInnermostRing: {r:outerRadius * 0, fill: 'grey'},
      ...
      'modern':{
          ...
          secondsRing: {r:outerRadius * 0.05, fill: 'gray'},
          secondsInnermostRing: {r:outerRadius * 0.05/2, fill: 'gray'},
          tickUnit: outerRadius * 0.0625/3,
          tickWidth: function(i) {
          ...
          clockHandOpacity: function(d) { // added to hide second hand
            if (d.unit === "hours") {
              return 1;
            } else if (d.unit === "minutes") {
              return 1;
  ...
  render = function(data) {
    ...
    if (!clockHand[0][0]) {
      //draw background
      clockGroup.append("svg:circle").attr("id","clockbg")
      	  .data(data)
      	  .attr("fill", function(d) { return d.numeric > 0 && d.numeric < 12 ? "white" : "black"; })
      	  .attr("opacity", 0.3)
      	  .attr("r", "60")
      //show the AM/PM
      clockGroup.append("svg:text")
          .data(data)
          .text(function(d) { console.log(d); return d.numeric > 0 && d.numeric < 12 ? "AM" : "PM"; })
          .style("fill", "white")
          .attr("x", -10)
          .attr("y", -20);
      ...
      .attr("opacity", function(d) { return faceObj.clockHandOpacity(d); })
    ...
    clockGroup.select("text")
    	.data(data)
    	.text(function(d) { return (d.numeric > 0 && d.numeric < 12) ? "AM" : "PM"; } );
    clockGroup.select("#clockbg")
    	.data(data)
    	.transition().duration(250)
    	.attr("fill", function(d) { 
    		return d.numeric > 6 && d.numeric < 20 ? "orange" : "black"; 
    	});
	
    clockHand.transition().duration(250).ease(faceObj.easing).attr("transform", function(d,i) {
    if (d.unit==="hours"){
